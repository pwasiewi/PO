...
public:
	T value;
	Number(T  v): Node<T>(INT), value(v){cout << "Powstaje obiekt Number o wartości: " << value << endl;}
	T eval(){
		return value;
	}
	~Number(){
		cout << "Usuwam obiekt Number o wartości: " << value << endl;
	}
};
...
int main ()
{
	...
	//Plus C(new Minus (new Number(3)), new Number(4));
	Node<float> *C = new Minus<float> (new Number<float>(3.3));
	Node<float> *D = new Number<float>(4.2);
	cout << C << " " << D << endl;
	Node<float> *E = *C + *D;
	cout << E << endl;
	delete E;
