...
int main ()
{
	...
	unique_ptr<Node<float>> C1(new Number<float>(1.1));
	unique_ptr<Node<float>> C(new Minus<float> (C1.get()));
	unique_ptr<Node<float>> D(new Number<float>(4.2));
	cout << *C << " " << *D << endl;
	Node<float> *E = *C + *D;
	cout << *E << endl;
	//nie potrzeba delete E, kiedy unique_ptr
	cout << "KONIEC" << endl;
}
